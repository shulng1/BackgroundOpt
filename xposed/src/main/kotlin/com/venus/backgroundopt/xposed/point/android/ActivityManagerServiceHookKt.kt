...
import com.venus.backgroundopt.xposed.hook.generateMatchedMethodHookPoint
 ...
 * @date 2023/8/19
 */
class ActivityManagerServiceHookKt(
...
//            ),
            generateMatchedMethodHookPoint(
                effectiveFlag = OsUtils.isSOrHigher,
                ClassConstants.ActivityManagerService,
            ...
            generateMatchedMethodHookPoint(
                effectiveFlag = OsUtils.isR,
                ClassConstants.ActivityManagerService,
                MethodConstants.removePidLocked,
                arrayOf(
                    beforeHookAction { handleRemovePidLockedUntilA11(it) }
                )
            ),
            HookPoint(
                ClassConstants.ActivityManagerService,
    ...
    private fun handleRemovePidLocked(param: MethodHookParam) {
        val pid = param.args[0] as Int

        runningInfo.removeProcess(pid)
    }

    private fun handleRemovePidLockedUntilA11(param: MethodHookParam) {
        val pid = ProcessRecord.getPid(param.args[0])
