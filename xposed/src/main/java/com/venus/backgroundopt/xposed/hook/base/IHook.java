...
public abstract class IHook implements ILogger {
    ...
    public IHook(@NonNull ClassLoader classLoader) {
        this(classLoader, new RunningInfo(classLoader));
    }

    public IHook(@NonNull ClassLoader classLoader, @NonNull RunningInfo runningInfo) {
    ...
    /**
     * 执行hook操作 <br>
     * 主要针对{@link XC_MethodHook.Unhook}, 在取消后再次设置
     */
    public void doHook() {
    ...
    public static void addMethodHookPoint(@NonNull String tag, @NonNull Set<XC_MethodHook.Unhook> set) {
        ...
        int hookMethodCount = set.size();
        if (hookMethodCount == 0) {
            ILogger.getLoggerStatic(IHook.class).warn("[hook失败] ->>> tag: [" + tag + "] 未hook到任何方法, 对应功能无法生效, 请带着日志联系开发者解决");
        } else {
            if (BuildConfig.DEBUG) {
                ILogger.getLoggerStatic(IHook.class).debug("添加 [" + tag + "] 的hook. 本次添加的set大小: " + hookMethodCount + "当前tag所属list大小为: " + list.size());
            }
        }
    }
